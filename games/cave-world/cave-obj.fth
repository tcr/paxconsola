...
: print4 >r >r >r >r >r >r r> dup print r> dup print r> dup print r> dup print r> dup print r> dup print ;
...
: json-parse-value ( c-addr1 u1 -- c-addr2 u2 value )
    json-trim
    dup 0x30 0x3a within if json-ungetchar ." @@@@@@@ungetchar\n" print4 json-parse-number 
    else
        case
...
: json-parse-object-member ( c-addr1 u1 -- c-addr2 u2 value key-addr key-u )
    ...
    print4
    ." \n     ^^^^^^^^^^\n"
    -rot        \ key-addr key-u value c-addr c-u
    ." \n     vvvvvvvvv TWO\n"
    print4
    ." \n     ^^^^^^^^^ TWO\n"
    json-trim case
        0x7d of 0x7d json-ungetchar ." !!!!!!!!!ungetchar" cr endof
        0x2c of endof            \ , is expected here
        json-parse-error throw   \ something else
    endcase                      \ key-addr key-u value c-addr c-u
    cr
    ."     [json-parse-object-member]" cr
    print4
    ."     [/json-parse-object-member]" cr cr
    rot 4 roll 4 roll ;
...
: json-parse-number-digits ( c-addr1 u1 -- c-addr2 u2 int )
    ...
    print4
    ." \n%%%%%%%%%%%%%%%\n"
    dup 0>= if json-ungetchar ."    unnnnnngetchar\n" else drop endif \ num c-addr u
    rot
    print4
    ;
...
:noname ( c-addr1 u1 -- c-addr2 u2 object )
    ...
    while
            ...
            r@
            ."      before map-set" cr print4

            map-set r!        \ c-addr u key-addr
...
s" applebees\" no thanks" 2dup type cr json-parse-string-body print print print print
...
mymap @
s" a" rot map-get json-data @ print
mymap @
s" beta" rot map-get json-data 2@ type cr
mymap @
s" arr" rot map-get json-data 2@ print 4 + @ @ print
